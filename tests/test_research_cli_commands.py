...
def test_cmd_alpha_research_accepts_multiple_signal_families_from_config(monkeypatch, tmp_path: Path) -> None:
    captured: dict[str, object] = {}
    config_path = tmp_path / "alpha_research.yaml"
    config_path.write_text(
        f"""
...
selection:
  symbols:
    - AAPL
signals:
  families: [momentum, fundamental_value]
  lookbacks: [5]
  horizons: [1]
tracking:
  tracker_dir: {(tmp_path / "tracking").as_posix()}
""".strip(),
    ...
    assert captured["signal_family"] == "momentum"
    assert captured["signal_families"] == ["momentum", "fundamental_value"]


def test_cmd_alpha_research_passes_fundamental_feature_settings(monkeypatch, tmp_path: Path) -> None:
    captured: dict[str, object] = {}
    ...
    args = argparse.Namespace(
        ...
        regime_exclude_mean_rank_ic=-0.01,
        experiment_tracker_dir=None,
        _cli_argv=["--symbols", "AAPL", "MSFT", "--fundamentals-enabled"],
    )
