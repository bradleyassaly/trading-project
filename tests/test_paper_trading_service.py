...
def test_generate_rebalance_orders_candidate_training_source_falls_back_to_executed(monkeypatch) -> None:
    ...
    def fake_build(**kwargs):
        calls.append((kwargs["training_source"], kwargs["target_type"], kwargs["hybrid_alpha"]))
    ...
    result = generate_rebalance_orders(
        ...
        config=PaperTradingConfig(
            ...
            ev_gate_target_type="hybrid_proxy",
            ev_gate_hybrid_alpha=0.7,
    ...
    assert calls == [
        ("candidate_decisions", "hybrid_proxy", 0.7),
        ("executed_trades", "hybrid_proxy", 0.7),
    ]
    assert result.diagnostics["ev_gate_training_source"] == "executed_trades"
    assert result.diagnostics["ev_gate_target_type"] == "hybrid_proxy"
    assert result.diagnostics["ev_gate_hybrid_alpha"] == pytest.approx(0.7)
    assert result.diagnostics["ev_gate_training_summary"]["fallback_reason"] == "insufficient_candidate_history_for_ev_gate"


def test_generate_rebalance_orders_regression_model_type_falls_back_to_bucketed_for_execution(monkeypatch) -> None:
    ...
    def fake_train(**kwargs):
        train_calls.append(str(kwargs["model_type"]))
        return {"training_available": True, "training_sample_count": 1}

    monkeypatch.setattr("trading_platform.paper.service.train_trade_ev_model", fake_train)
    monkeypatch.setattr(
        "trading_platform.paper.service.score_trade_ev_candidates",
        lambda **kwargs: [
            {
    ...
    assert train_calls == ["bucketed_mean"]
    assert result.diagnostics["ev_gate_requested_model_type"] == "regression"
    assert result.diagnostics["ev_gate_model_type"] == "bucketed_mean"
    assert (
        result.diagnostics["ev_gate_training_summary"]["fallback_reason"]
        == "regression_model_is_diagnostics_only_for_now"
    )


def test_generate_rebalance_orders_regression_soft_mode_uses_regression_weighting_when_available(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
        ...
        lambda **kwargs: [
            {
                **kwargs["candidate_rows"][0],
                ...
                "probability_positive": 0.6,
                "raw_ev_score": 0.01,
                "normalized_ev_score": 0.01,
                "ev_score_pre_clip": 0.01,
                "ev_score_post_clip": 0.01,
                "ev_score_clipped": False,
    ...
    monkeypatch.setattr(
        "trading_platform.paper.service.score_trade_ev_regression_candidates",
        lambda **kwargs: [
            {
                "symbol": "AAPL",
                "predicted_ev": 0.03,
                ...
                "regression_normalized_ev_score": 0.20,
                "regression_ev_score_post_clip": 0.20,
                "ev_weighting_score": 0.20,
            }
        ],
    )
    ...
    assert len(result.orders) == 1
    assert result.orders[0].target_weight == pytest.approx(0.32)
    assert result.orders[0].provenance["ev_model_type_used"] == "regression"
    assert result.orders[0].provenance["regression_raw_ev_score"] == pytest.approx(0.03)
    assert result.diagnostics["ev_model_type_used"] == "regression"
    assert result.diagnostics["regression_prediction_available_count"] == 1
    assert result.diagnostics["avg_regression_ev_weighting_score"] == pytest.approx(0.20)


def test_generate_rebalance_orders_regression_soft_mode_applies_confidence_weighting(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
        ...
        lambda **kwargs: [
            {
                ...
                "regression_ev_score_post_clip": 0.20,
                "ev_confidence": 0.8,
                "ev_confidence_multiplier": 1.25,
                "ev_score_before_confidence": 0.20,
                "ev_score_after_confidence": 0.25,
                "ev_weighting_score": 0.25,
            }
        ],
    )
    ...
    result = generate_rebalance_orders(
        ...
        config=PaperTradingConfig(
            ...
            ev_gate_training_root="artifacts/daily_replay/run_current",
            ev_gate_min_training_samples=1,
            ev_gate_use_confidence_weighting=True,
            ev_gate_confidence_method="residual_std",
            ev_gate_confidence_scale=1.0,
            ev_gate_confidence_clip_min=0.5,
            ev_gate_confidence_clip_max=1.5,
            min_trade_dollars=1.0,
        ),
        min_trade_dollars=1.0,
    )
    ...
    assert len(result.orders) == 1
    assert result.orders[0].target_weight == pytest.approx(0.35)
    assert result.orders[0].provenance["ev_confidence"] == pytest.approx(0.8)
    assert result.orders[0].provenance["ev_confidence_multiplier"] == pytest.approx(1.25)
    assert result.orders[0].provenance["ev_score_before_confidence"] == pytest.approx(0.20)
    assert result.orders[0].provenance["ev_score_after_confidence"] == pytest.approx(0.25)
    assert result.diagnostics["avg_ev_confidence"] == pytest.approx(0.8)
    assert result.diagnostics["avg_ev_confidence_multiplier"] == pytest.approx(1.25)
    assert result.diagnostics["avg_ev_score_after_confidence"] == pytest.approx(0.25)


def test_generate_rebalance_orders_regression_soft_mode_can_filter_low_confidence(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
        ...
        lambda **kwargs: [
            {
                ...
                "regression_ev_score_post_clip": 0.20,
                "ev_confidence": 0.2,
                "ev_confidence_multiplier": 0.75,
                "ev_score_before_confidence": 0.20,
                "ev_score_after_confidence": 0.15,
                "ev_weighting_score": 0.15,
            }
        ],
    )
    ...
    result = generate_rebalance_orders(
        ...
        config=PaperTradingConfig(
            ...
            ev_gate_training_root="artifacts/daily_replay/run_current",
            ev_gate_min_training_samples=1,
            ev_gate_use_confidence_filter=True,
            ev_gate_confidence_threshold=0.5,
            min_trade_dollars=1.0,
        ),
        min_trade_dollars=1.0,
    )
    ...
    assert result.orders == []
    assert result.diagnostics["confidence_filtered_count"] == 1
    assert result.diagnostics["candidate_trade_rows"][0]["candidate_outcome"] == "confidence_filtered"


def test_generate_rebalance_orders_regression_soft_mode_applies_reliability_weighting(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {"training_available": True, "training_sample_count": 12},
    )
    monkeypatch.setattr(
        "trading_platform.paper.service.score_trade_ev_reliability_candidates",
        lambda **kwargs: [
            {
                "symbol": "AAPL",
                "prediction_available": True,
...
def test_generate_rebalance_orders_regression_soft_mode_can_filter_low_reliability(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
    ...
    assert result.orders == []
    assert result.diagnostics["reliability_filtered_count"] == 1
    assert result.diagnostics["candidate_trade_rows"][0]["candidate_outcome"] == "reliability_filtered"


def test_generate_rebalance_orders_regression_soft_mode_falls_back_when_prediction_missing(monkeypatch) -> None:
    monkeypatch.setattr(
        "trading_platform.paper.service.build_trade_ev_training_dataset",
        lambda **kwargs: ([{"forward_net_return": 0.02}], {"training_sample_count": 1}),
    )
    ...
    assert len(result.orders) == 1
    assert result.orders[0].target_weight == pytest.approx(0.212)
    assert result.orders[0].provenance["ev_model_type_used"] == "bucketed_mean"
    assert result.diagnostics["ev_model_fallback_reason"] == "regression_predictions_unavailable"
...
def test_generate_rebalance_orders_regression_hard_mode_does_not_use_regression_predictions(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "trading_platform.paper.service.score_trade_ev_regression_candidates",
        lambda **kwargs: pytest.fail("regression scoring should not run in hard mode"),
    )
    ...
    assert len(result.orders) == 1
    assert result.diagnostics["ev_model_type_used"] == "bucketed_mean"
