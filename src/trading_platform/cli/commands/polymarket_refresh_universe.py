...
def cmd_polymarket_refresh_universe(args: argparse.Namespace) -> None:
    ...
    if full:
        min_volume = getattr(args, "min_volume", 10_000)
        print(f"Full coverage refresh (min_volume=${min_volume:,})...")
        universe.refresh_full_coverage(min_volume=min_volume)
    else:
        max_per_cat = getattr(args, "max_per_category", 50)
