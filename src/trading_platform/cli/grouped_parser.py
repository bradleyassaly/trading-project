...
def _add_alpha_research_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--require-composite-runtime-computability-for-approval", action="store_true", help="Block approval-ready composite states that cannot form current composite scores.")
    parser.add_argument("--min-composite-runtime-computable-symbols-for-approval", type=int, default=5, help="Minimum symbols with current composite scores required for composite approval readiness.")
    parser.add_argument("--allow-research-only-noncomputable-composites", action="store_true", help="Keep non-computable composites as research-only diagnostics while blocking approval in strict mode.")
    parser.add_argument("--composite-runtime-computability-check-mode", type=str, default="strict", choices=["strict", "penalize", "diagnostic_only"], help="How composite runtime computability affects approval readiness.")
    parser.add_argument("--composite-runtime-computability-penalty-on-ranking", type=float, default=0.02, help="Penalty applied to composite approval ranking when composite runtime computability check mode is penalize.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    data_refresh_inputs.add_argument("--fundamentals-vendor-api-key", type=str, default=None, help="Optional FMP vendor API key. Falls back to FMP_API_KEY when omitted.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-cache-enabled", action=argparse.BooleanOptionalAction, default=None, help="Enable or disable raw FMP response caching.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-cache-root", type=str, default=None, help="Optional raw FMP cache root. Defaults under the fundamentals artifact root.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-cache-ttl-hours", type=float, default=None, help="Freshness TTL for cached FMP responses.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-force-refresh", action="store_true", help="Bypass fresh cache entries and refetch vendor data.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-request-delay-seconds", type=float, default=None, help="Throttle delay between FMP requests.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-max-retries", type=int, default=None, help="Maximum FMP retry attempts for 429/transient failures.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-max-symbols-per-run", type=int, default=None, help="Optional cap on symbols fetched from FMP in one run.")
    data_refresh_inputs.add_argument("--fundamentals-vendor-max-requests-per-run", type=int, default=None, help="Optional cap on raw FMP requests in one run.")
    ...
    data_fundamentals_ingest.add_argument("--vendor-api-key", "--fundamentals-vendor-api-key", dest="vendor_api_key", type=str, default=None, help="Optional FMP vendor API key. Falls back to FMP_API_KEY when omitted.")
    data_fundamentals_ingest.add_argument("--vendor-cache-enabled", action=argparse.BooleanOptionalAction, default=True, help="Enable or disable raw FMP response caching.")
    data_fundamentals_ingest.add_argument("--vendor-cache-root", type=str, default=None, help="Optional raw FMP cache root. Defaults under the fundamentals artifact root.")
    data_fundamentals_ingest.add_argument("--vendor-cache-ttl-hours", type=float, default=24.0, help="Freshness TTL for cached FMP responses.")
    data_fundamentals_ingest.add_argument("--vendor-force-refresh", action="store_true", help="Bypass fresh cache entries and refetch vendor data.")
    data_fundamentals_ingest.add_argument("--vendor-request-delay-seconds", type=float, default=0.5, help="Throttle delay between FMP requests.")
    data_fundamentals_ingest.add_argument("--vendor-max-retries", type=int, default=4, help="Maximum FMP retry attempts for 429/transient failures.")
    ...
    research_promote.add_argument("--run-dir", type=str, default=None, help="Optional research run directory to promote from. Defaults to the latest run under --artifacts-root when using run-local promotion.")
    research_promote.add_argument("--registry-scope", type=str, default="run_local", choices=["run_local", "global"], help="Promotion source scope. Defaults to run-local registry resolution to avoid mixing stale historical runs.")
    research_promote.add_argument("--use-global-registry", action="store_true", help="Legacy convenience flag that sets --registry-scope global.")
    research_promote.add_argument("--registry-dir", type=str, default=None, help="Optional directory where refreshed research registry and promotion-candidate artifacts will be written. Defaults to <run-dir>/research_registry for run-local scope or <artifacts-root>/research_registry for global scope.")
