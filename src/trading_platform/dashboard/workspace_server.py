...
def _detail_href(
    base_path: str, identifier: object, page_state: PageState | None = None, **extra_query: str | None
) -> str:
...
def _render_value(column: str, value: object, row: dict | None = None, page_state: PageState | None = None) -> str:
    ...
    if column.endswith("status") or column == "status":
        return _badge(value)
    if column.endswith("_pnl") or column in {
        "equity",
        "cash",
        "market_value",
...
def _table(
    columns: list[str], rows: list[dict], *, empty: str = "No data available.", page_state: PageState | None = None
) -> str:
    if not rows:
    ...
    for row in rows:
        ...
        body.append(
            "<tr>"
            + "".join(
                f"<td>{_render_value(column, mapping.get(column), mapping, page_state)}</td>" for column in columns
            )
            + "</tr>"
        )
    ...
    for field in fields:
        ...
        if field_type == "select":
            ...
            for option in field.get("options", []):
                ...
                options.append(
                    f"<option value='{html.escape(option_value)}'{selected}>{html.escape(option_value)}</option>"
                )
...
def _pagination_controls(page_state: PageState, pagination: dict[str, object]) -> str:
    ...
    if offset + limit < total_count:
        next_link = (
            f"<a class='button-link secondary' href='{html.escape(page_state.url(offset=offset + limit))}'>Next</a>"
        )
    return (
...
def _overview_page(service: DashboardDataService) -> bytes:
    ...
    body += _info_cards(
        [
            (
                "Latest Run",
                overview.get("latest_run", {}).get("run_name") or "n/a",
                _freshness_badge(timestamp=overview.get("latest_run", {}).get("started_at")),
            ),
            ("Monitoring", overview.get("monitoring", {}).get("status") or "n/a", "Latest run health state"),
            ("Open Trades", blotter.get("summary", {}).get("open_trade_count") or 0, "Current blotter state"),
        ]
    )
    body += "</div></section>"
    body += _section(
        ...
        _metric_cards(
            [
                (
                    "Equity / Return",
                    _format_number(overview.get("system_evaluation", {}).get("total_return"), pct=True),
                    "Latest evaluated system return",
                ),
                ("Sharpe", _format_number(overview.get("system_evaluation", {}).get("sharpe")), "System evaluation"),
                ("Open Trades", blotter.get("summary", {}).get("open_trade_count") or 0, "Trade blotter"),
                ("Closed Trades", blotter.get("summary", {}).get("closed_trade_count") or 0, "Trade blotter"),
                (
                    "Portfolio Positions",
                    overview.get("portfolio", {}).get("generated_position_count") or 0,
                    "Latest allocation output",
                ),
                (
                    "Executable Orders",
                    overview.get("execution", {}).get("executable_order_count") or 0,
                    "Current execution package",
                ),
                ("Approved Strategies", overview.get("registry", {}).get("approved_strategy_count") or 0, "Registry"),
                ("Critical Alerts", ops.get("summary", {}).get("critical_alert_count") or 0, "Latest run"),
            ]
        ),
    )
    body += _section(
        ...
        _table(
            ...
            blotter.get("trades", [])[:8],
            empty="No recent trades found.",
        ),
        subtitle="Recent proposed, open, and closed trades with direct drill-down links.",
    )
    body += _section(
        "Recent Symbols",
        _table(
            ...
            empty="No recent symbols found.",
        ),
    )
    body += _section(
        "Open Positions",
        _table(
            ["symbol", "side", "qty", "avg_price", "market_value"],
            service.portfolio_overview_payload().get("positions", [])[:8],
    ...
    body += _section(
        ...
        _table(
            ...
            empty="No strategy activity found.",
        ),
    )
...
def _trades_page(service: DashboardDataService, query: dict[str, list[str]]) -> bytes:
    page_state = _page_state(
        "/trades",
        query,
        filter_keys=["status", "strategy", "symbol", "run_id", "date_from", "date_to"],
        default_limit=50,
    )
    ...
    body = _metric_cards(
        [
            ("Trade Count", payload.get("summary", {}).get("trade_count") or 0, "Across discovered explicit ledgers"),
            ("Open", payload.get("summary", {}).get("open_trade_count") or 0, "Currently active"),
            ("Closed", payload.get("summary", {}).get("closed_trade_count") or 0, "Completed trades"),
            ("Page Size", pagination.get("limit") or len(rows), "Requested slice"),
            ("Source", payload.get("source") or "n/a", "Read path"),
        ]
    )
    body += _filter_form(
        page_state,
        [
            ...
            {"name": "strategy", "label": "Strategy", "placeholder": "momentum-core"},
            {
                ...
                "label": "Status",
                "type": "select",
                "options": ["open", "closed", "selected", "rejected", "failed"],
            },
    ...
    body += _section(
        ...
        subtitle="The blotter centers the platform on inspectable trades rather than raw artifact files.",
    )
...
def _timeline(rows: list[dict]) -> str:
    ...
    for row in rows:
        items.append(
            ...
            f"<div class='timeline-title'><strong>{html.escape(str(row.get('label') or row.get('kind') or 'event'))}</strong>{_badge(row.get('status'))}</div>"
            f"<div class='subtle'>{_escape(row.get('ts'))}</div><div class='metric-detail'>{_escape(row.get('detail'))}</div></div>"
        )
...
def _trade_detail_page(service: DashboardDataService, trade_id: str, query: dict[str, list[str]]) -> bytes:
    ...
    if not trade:
        return _page_shell(
            ...
            active_path="/trades",
            body="<section class='panel'><div class='empty'>Trade not found.</div></section>",
        )
    ...
    body += _info_cards(
        [
            ("Status", trade.get("status") or "n/a", "Lifecycle state"),
            ("Strategy", trade.get("strategy_id") or "n/a", "Generating strategy"),
            (
                "Run",
                payload.get("related_metadata", {}).get("run_id") or "latest",
                _freshness_badge(path=payload.get("related_metadata", {}).get("trade_source")),
            ),
        ]
    )
    body += "</div></section>"
    body += _section(
        ...
        _metric_cards(
            [
                ...
                ("Quantity", trade.get("qty") or 0, "Ledger size"),
                ("Entry", trade.get("entry_price") or "n/a", _escape(trade.get("entry_ts"))),
                ("Exit", trade.get("exit_price") or "n/a", _escape(trade.get("exit_ts"))),
                ("Realized PnL", _format_number(trade.get("realized_pnl"), money=True), "Closed outcome"),
                ("Hold (hrs)", _format_number(trade.get("hold_duration_hours")), "Entry to exit"),
            ]
        ),
    )
    body += _section(
        ...
        _info_cards(
            [
                (
                    "Signal / Trigger",
                    signal.get("label") or signal.get("type") or "Not available",
                    "Nearest signal before entry",
                ),
                ("Signal Score", signal.get("score") or "n/a", "From current artifacts if present"),
                (
                    "Ranking Score",
                    payload.get("provenance", {}).get("ranking_score") or "n/a",
                    "Selection/ranking evidence",
                ),
                (
                    "Universe Rank",
                    payload.get("provenance", {}).get("universe_rank") or "n/a",
                    "Relative candidate rank",
                ),
                (
                    "Target Weight",
                    payload.get("portfolio_context", {}).get("target_weight") or "n/a",
                    "Portfolio sizing target",
                ),
                (
                    "Constraint Hits",
                    ", ".join(payload.get("portfolio_context", {}).get("constraint_hits", [])) or "None recorded",
                    "Constraint-aware selection notes",
                ),
            ]
        )
        + _table(
            ...
            empty="No decision provenance rows found.",
        ),
        subtitle="Evidence is sourced from available signal and provenance artifacts only.",
    )
    body += _section(
        "Portfolio Context",
        _info_cards(
            [
                (
                    "Selection Status",
                    payload.get("portfolio_context", {}).get("selection_status") or "n/a",
                    "Included/excluded at the decision point",
                ),
                (
                    "Selected Among Alternatives",
                    payload.get("portfolio_context", {}).get("selected_among_alternatives"),
                    f"Candidate count: {_escape(payload.get('portfolio_context', {}).get('candidate_count'))}",
                ),
                (
                    "Current Portfolio Qty",
                    payload.get("portfolio_context", {}).get("portfolio_qty") or 0,
                    "Latest position snapshot",
                ),
                (
                    "Market Value",
                    _format_number(payload.get("portfolio_context", {}).get("portfolio_market_value"), money=True),
                    "Current portfolio context",
                ),
                (
                    "Unrealized PnL",
                    _format_number(payload.get("portfolio_context", {}).get("unrealized_pnl"), money=True),
                    "If still held",
                ),
                (
                    "Regime Context",
                    (explain.get("regime") or {}).get("regime_label") or "n/a",
                    "Latest market regime artifact",
                ),
            ]
        ),
    )
    body += _section(
        "Trade Chart",
        _trade_chart(payload.get("chart", {}), trade),
        subtitle="Lightweight annotated price chart with entry, exit, fills, and nearby signals.",
    )
    body += _section(
        "Associated Signals",
        _table(
            ["ts", "type", "label", "price", "score"], payload.get("signals", []), empty="No associated signals found."
        ),
    )
    body += _section(
        "Execution Review",
        _info_cards(
            [
                (
                    "Order Count",
                    payload.get("execution_review", {}).get("order_count") or 0,
                    "Orders discovered in the trade window",
                ),
                (
                    "Fill Count",
                    payload.get("execution_review", {}).get("fill_count") or 0,
                    "Fills discovered in the trade window",
                ),
                (
                    "Executed Qty",
                    payload.get("execution_review", {}).get("executed_qty") or 0,
                    "Aggregate discovered fills",
                ),
                (
                    "Avg Fill Price",
                    payload.get("execution_review", {}).get("average_fill_price") or "n/a",
                    "Weighted by discovered fill quantity",
                ),
                (
                    "Latest Order Status",
                    payload.get("execution_review", {}).get("latest_order_status") or "n/a",
                    "Most recent order state",
                ),
                (
                    "Latest Fill Status",
                    payload.get("execution_review", {}).get("latest_fill_status") or "n/a",
                    "Most recent fill state",
                ),
            ]
        )
        + _table(
            ["ts", "side", "qty", "price", "order_id", "status", "reason"],
            payload.get("orders", []),
            empty="No associated orders found.",
        )
        + _table(
            ["ts", "side", "qty", "price", "order_id", "status"],
            payload.get("fills", []),
            empty="No associated fills found.",
        ),
    )
    ...
    body += _section(
        ...
        _info_cards(
            [
                ("Trade Status", payload.get("outcome_review", {}).get("trade_status") or "n/a", "Open vs closed"),
                (
                    "Realized PnL",
                    _format_number(payload.get("outcome_review", {}).get("realized_pnl"), money=True),
                    "Closed-trade result",
                ),
                (
                    "Unrealized PnL",
                    _format_number(payload.get("outcome_review", {}).get("unrealized_pnl"), money=True),
                    "Current mark-to-market if open",
                ),
                (
                    "Price Change",
                    _format_number(payload.get("outcome_review", {}).get("price_change")),
                    "Entry to exit price delta",
                ),
                (
                    "Holding Period",
                    _format_number(payload.get("outcome_review", {}).get("holding_period_hours")),
                    "Hours held",
                ),
                (
                    "Indicator Snapshot",
                    ", ".join(f"{key}={value}" for key, value in (explain.get("indicator_snapshot") or {}).items())
                    or "Not available",
                    "Features captured around entry",
                ),
            ]
        ),
    )
    ...
    body += _section(
        "Related Strategy / Run Metadata",
        _info_cards(
            [
                ("Source", payload.get("related_metadata", {}).get("source") or "n/a", "Artifact source"),
                ("Run ID", payload.get("related_metadata", {}).get("run_id") or "n/a", "Related run metadata"),
                ("Mode", payload.get("related_metadata", {}).get("mode") or "n/a", "Paper/live/research mode"),
                (
                    "Trade Source Mode",
                    payload.get("related_metadata", {}).get("trade_source_mode") or "n/a",
                    "Explicit vs derived",
                ),
                (
                    "Trade Source",
                    payload.get("related_metadata", {}).get("trade_source") or "n/a",
                    "Underlying artifact file",
                ),
                (
                    "Position Source",
                    payload.get("related_metadata", {}).get("position_source") or "n/a",
                    "Latest position artifact",
                ),
            ]
        )
        + _table(
...
def _strategies_page(service: DashboardDataService, query: dict[str, list[str]]) -> bytes:
    page_state = _page_state(
        "/strategies", query, filter_keys=["status", "strategy", "decision", "date_from", "date_to"], default_limit=20
    )
    payload = service.strategies_payload(page_state.to_service_filters())
    rows = payload.get("strategies", [])
    status_filter = page_state.filters.get("status")
    if status_filter:
        rows = [row for row in rows if row.get("status") == status_filter]
    body = _metric_cards(
        [
            ("Strategy Count", len(payload.get("strategies", [])), "Registry entries"),
            ("Approved", payload.get("summary", {}).get("status_counts", {}).get("approved", 0), "Production-approved"),
            ("Families", len(payload.get("summary", {}).get("family_counts", {})), "Observed families"),
            (
                "Under Review",
                payload.get("summary", {}).get("lifecycle_counts", {}).get("under_review", 0),
    ...
    body += _filter_form(
        ...
        [
            ...
            {"name": "decision", "label": "Promotion", "type": "select", "options": ["promote", "reject"]},
            {
                ...
                "label": "Status",
                "type": "select",
                "options": ["approved", "active", "inactive", "under_review"],
            },
            {"name": "date_from", "label": "Date From", "type": "date"},
            {"name": "date_to", "label": "Date To", "type": "date"},
        ],
    ...
    body += _section(
        "Strategy Registry",
        _table(
            ["strategy_id", "status", "family", "version", "preset_name", "universe", "current_deployment_stage"],
            rows,
            page_state=page_state,
        ),
    )
    body += _section(
        ...
        _table(
            [
                ...
                "adaptive_adjusted_weight",
                "latest_reasons",
            ],
            payload.get("strategy_lifecycle", []),
            empty="No lifecycle rows found.",
        ),
    )
    body += _pagination_controls(
        page_state,
        payload.get(
            "recent_promotions_pagination",
            {
                "total_count": len(payload.get("recent_promotions", [])),
    ...
    body += _section(
        ...
        _table(
            [
                ...
                "decision",
                "promoted_status",
                "source_research_run_name",
            ],
            payload.get("recent_promotions", []),
            empty="No DB-backed promotions found.",
            page_state=page_state,
        ),
    )
    body += _section(
        ...
        _table(
            (
                list(payload.get("champion_challenger", [{}])[0].keys())
                if payload.get("champion_challenger")
                else ["family"]
            ),
            payload.get("champion_challenger", []),
            empty="No champion/challenger mapping found.",
        ),
    )
...
def _strategy_detail_page(service: DashboardDataService, strategy_id: str) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Closed Trades", summary.get("closed_trade_count") or 0, "Explicit ledgers"),
            ("Open Trades", summary.get("open_trade_count") or 0, "Currently active"),
            ("Win Rate", _format_number(summary.get("win_rate"), pct=True), "Closed trades only"),
            ("Expectancy", _format_number(summary.get("expectancy"), money=True), "Average realized pnl"),
            ("Avg Hold (hrs)", _format_number(summary.get("average_holding_period_hours")), "Closed trades"),
            ("Realized PnL", _format_number(summary.get("cumulative_realized_pnl"), money=True), "Closed trades"),
        ]
    )
    body += _section(
        "Recent Trades",
        _table(
            ["trade_id", "symbol", "side", "qty", "entry_ts", "exit_ts", "realized_pnl", "status"],
            payload.get("trades", []),
    ...
    body += _section(
        ...
        _table(
            ...
            empty="No source comparisons found.",
        ),
    )
    body += _section(
        "PnL By Symbol",
...
def _portfolio_page(service: DashboardDataService) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Equity", _format_number(portfolio_summary.get("equity"), money=True), "Latest portfolio summary"),
            ("Cash", _format_number(portfolio_summary.get("cash"), money=True), "Latest portfolio summary"),
            ("Open Positions", portfolio_summary.get("open_position_count") or 0, "Current holdings"),
            (
                "Gross Exposure",
                _format_number(summary.get("gross_exposure_after_constraints"), pct=True),
                "After constraints",
            ),
            (
                "Net Exposure",
                _format_number(summary.get("net_exposure_after_constraints"), pct=True),
                "After constraints",
            ),
            (
                "Latest Drawdown",
                _format_number(portfolio_summary.get("latest_drawdown"), pct=True),
                "From equity curve",
            ),
        ]
    )
    ...
    body += _section(
        "Exposure By Symbol",
        _table(
            ["symbol", "side", "market_value", "weight_proxy"],
            overview.get("exposure", []),
            empty="No exposure rows found.",
        ),
    )
    body += _section(
        "Recent Activity",
        _table(
            ["kind", "ts", "symbol", "side", "qty", "price", "status"],
    ...
    body += _section(
        "Best Recent Trades",
        _table(
            ["trade_id", "symbol", "side", "qty", "realized_pnl", "entry_ts", "exit_ts", "strategy_id", "status"],
            overview.get("best_trades", []),
            empty="No winning realized trades found.",
        ),
    )
    body += _section(
        "Worst Recent Trades",
        _table(
            ["trade_id", "symbol", "side", "qty", "realized_pnl", "entry_ts", "exit_ts", "strategy_id", "status"],
            overview.get("worst_trades", []),
            empty="No losing realized trades found.",
        ),
    )
    body += _section(
        "Regime & Allocation Context",
        _table(
            ["preset_name", "prior_weight", "adjusted_weight", "delta_weight", "monitoring_recommendation"],
            payload.get("adaptive_allocation", {}).get("top_changes", []),
            empty="No adaptive allocation changes found.",
        ),
    )
...
def _research_page(service: DashboardDataService) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Research Runs", payload.get("summary", {}).get("run_count", 0), "Indexed research manifests"),
            ("Eligible Candidates", payload.get("summary", {}).get("eligible_candidate_count", 0), "Promotion ready"),
            (
                "Promoted Strategies",
                payload.get("summary", {}).get("promoted_strategy_count", 0),
                "Generated strategy presets",
            ),
            (
                "Validated Pass",
                payload.get("strategy_validation", {}).get("summary", {}).get("pass_count", 0),
    ...
    body += _section(
        ...
        _table(
            ...
            empty="No promotion candidates found.",
        ),
    )
    body += _section(
        "Recent Promotions",
        _table(
            ...
            payload.get("recent_promotions", []),
            empty="No DB-backed promotion history found.",
        ),
    )
    body += _section(
        ...
        _table(
            ...
            empty="No selected strategy portfolio found.",
        ),
    )
...
def _ops_page(service: DashboardDataService, query: dict[str, list[str]]) -> bytes:
    page_state = _page_state(
        "/ops", query, filter_keys=["status", "activity_type", "date_from", "date_to"], default_limit=20
    )
    payload = service.ops_payload(page_state.to_service_filters())
    latest_run = payload.get("latest_run", {})
    body = _metric_cards(
        [
            ("Latest Run", payload.get("summary", {}).get("latest_run_name") or "n/a", "Most recent pipeline"),
            ("Run Status", payload.get("summary", {}).get("latest_run_status") or "n/a", "Most recent pipeline"),
            ("Health", payload.get("summary", {}).get("health_status") or "n/a", "Latest run health"),
            ("Critical Alerts", payload.get("summary", {}).get("critical_alert_count") or 0, "Latest run"),
            ("Blocked Checks", payload.get("summary", {}).get("blocked_check_count") or 0, "Latest live submission"),
            ("Missing Fills", payload.get("summary", {}).get("missing_fill_count") or 0, "Execution diagnostics"),
        ]
    )
    body += _filter_form(
        page_state,
        [
            {"name": "status", "label": "Status", "type": "select", "options": ["completed", "failed", "running"]},
            {
                ...
                "label": "Activity",
                "type": "select",
                "options": ["portfolio_run", "research_run", "trade_decision", "promotion", "execution"],
            },
            {"name": "date_from", "label": "Date From", "type": "date"},
            {"name": "date_to", "label": "Date To", "type": "date"},
        ],
    ...
    body += _section(
        "Latest Run Detail",
        _table(
            ["stage_name", "status", "started_at", "ended_at", "duration_seconds", "error_message"],
            latest_run.get("stages", []),
            empty="No stage records found.",
        ),
    )
    body += _pagination_controls(
        page_state,
        payload.get(
            "runs_pagination",
            {
                "total_count": len(payload.get("runs", [])),
                "limit": page_state.limit,
                "offset": page_state.offset,
                "source": payload.get("source"),
            },
    ...
    body += _section(
        ...
        _table(
            ...
            payload.get("db_activity", {}).get("activity_feed", {}).get("items", []),
            empty="No DB activity found.",
            page_state=page_state,
        ),
    )
    body += _section(
        ...
        _table(
            ...
            payload.get("orchestration_runs", []),
    ...
    body += _section(
        "Live Risk Checks",
        _table(
            ["check_name", "passed", "hard_block", "severity", "message"],
            payload.get("live", {}).get("risk_checks", []),
            empty="No live risk checks found.",
        ),
    )
    body += _section(
        ...
        _table(
            ...
            payload.get("execution_diagnostics", {}).get("rows", []),
            empty="No execution diagnostics found.",
        ),
    )
...
def _live_page(service: DashboardDataService) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Dry-Run Orders", payload.get("dry_run_summary", {}).get("adjusted_order_count", 0), "Latest preview"),
            (
                "Submitted Orders",
                payload.get("submission_summary", {}).get("submitted_order_count", 0),
                "Latest submission",
            ),
            (
                "Duplicate Skips",
                payload.get("submission_summary", {}).get("duplicate_order_skip_count", 0),
                "Idempotency protection",
            ),
            (
                "Broker Health",
                payload.get("broker_health", {}).get("status") or "n/a",
                payload.get("broker_health", {}).get("message") or "Not available",
            ),
        ]
    )
    body += _section(
        ...
        _table(
            ["check_name", "passed", "hard_block", "severity", "message"],
            payload.get("risk_checks", []),
            empty="No risk checks found.",
        ),
    )
    body += _section(
        "Blocked Checks",
        _table(
            ["check_name", "severity", "message"], payload.get("blocked_checks", []), empty="No blocked checks found."
        ),
    )
...
def _execution_page(service: DashboardDataService) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Requested Orders", payload.get("summary", {}).get("requested_order_count", 0), "Before constraints"),
            ("Executable Orders", payload.get("summary", {}).get("executable_order_count", 0), "After constraints"),
            ("Rejected Orders", payload.get("summary", {}).get("rejected_order_count", 0), "Hard failures"),
            (
                "Expected Cost",
                _format_number(payload.get("summary", {}).get("expected_total_cost"), money=True),
                "Fees + slippage",
            ),
            (
                "Avg Latency",
                _format_number(diagnostics.get("summary", {}).get("average_signal_to_fill_latency_seconds")),
                "Seconds",
            ),
            ("Avg Slippage", _format_number(diagnostics.get("summary", {}).get("average_slippage_bps")), "Bps"),
        ]
    )
    body += _section(
        "Artifact Sources",
        _info_cards(
            [
                (
                    "Orders Source",
                    diagnostics.get("meta", {}).get("orders_source") or "n/a",
                    _freshness_badge(path=diagnostics.get("meta", {}).get("orders_source")),
                ),
                (
                    "Fills Source",
                    diagnostics.get("meta", {}).get("fills_source") or "n/a",
                    _freshness_badge(path=diagnostics.get("meta", {}).get("fills_source")),
                ),
                (
                    "Rejected Source",
                    diagnostics.get("meta", {}).get("rejected_source") or "n/a",
                    _freshness_badge(path=diagnostics.get("meta", {}).get("rejected_source")),
                ),
            ]
        ),
    )
    body += _section(
        ...
        _table(
            ...
            payload.get("executable_orders", []),
            empty="No executable orders found.",
        ),
    )
    body += _section(
        "Rejected Orders",
        _table(
            ["symbol", "side", "requested_shares", "rejection_reason"],
            payload.get("rejected_orders", []),
            empty="No rejected orders found.",
        ),
    )
    body += _section(
        "Execution Diagnostics",
...
def _runs_page(service: DashboardDataService, query: dict[str, list[str]]) -> bytes:
    page_state = _page_state(
        "/runs",
        query,
        filter_keys=["status", "run_kind", "run_type", "mode", "strategy", "date_from", "date_to"],
        default_limit=20,
    )
    ...
    body = _metric_cards(
        [
            (
                "Portfolio Runs",
                payload.get("runs_pagination", {}).get("total_count") or len(runs),
                "Normalized run history",
            ),
            (
                "Research Runs",
                payload.get("research_runs_pagination", {}).get("total_count") or len(research_runs),
                "Research lineage",
            ),
            ("Source", payload.get("source") or "n/a", "Read path"),
            ("Page Size", page_state.limit, "Requested slice"),
        ]
    )
    body += _filter_form(
        page_state,
    ...
    body += _section(
        "Portfolio Runs",
        _table(
            ["run_id", "run_name", "status", "run_type", "mode", "started_at", "completed_at", "artifact_count"],
            runs,
            empty="No runs found.",
            page_state=page_state,
        ),
    ...
    body += _section(
        "Research Runs",
        _table(
            ["run_id", "run_name", "status", "run_type", "started_at", "completed_at", "artifact_count"],
            research_runs,
            empty="No research runs found.",
            page_state=page_state,
        ),
    )
...
def _run_detail_page(service: DashboardDataService, run_id: str, query: dict[str, list[str]]) -> bytes:
    ...
    body = _metric_cards(
        [
            ("Run", summary.get("run_name") or "n/a", "Run key"),
            ("Status", summary.get("status") or "n/a", "Lifecycle"),
            ("Artifacts", summary.get("artifact_count") or 0, "Linked artifact rows"),
            ("Source", payload.get("source") or "n/a", "Read path"),
        ]
    )
    ...
    body += _section(
        ...
        _info_cards(
            [
                ("Run ID", summary.get("run_id") or run_id, "Stable DB id"),
                ("Run Kind", summary.get("run_kind") or "n/a", "Research vs portfolio"),
                ("Started", summary.get("started_at") or "n/a", "Start time"),
                ("Completed", summary.get("completed_at") or "n/a", "Completion time"),
                ("Artifact Dir", summary.get("artifact_dir") or "n/a", "Linked artifact root"),
                ("Git Commit", summary.get("git_commit") or "n/a", "Recorded at run start"),
            ]
        ),
    )
    body += _section(
        "Linked Artifacts",
        _table(
            ["role", "artifact_type", "format", "row_count", "path"],
            payload.get("artifacts", []),
            empty="No linked artifacts found.",
        ),
    )
    body += _section(
        "Linked Decisions",
        _table(
            ["trade_id", "symbol", "side", "strategy_id", "ranking_score", "order_status", "status"],
            payload.get("linked_decisions", {}).get("items", []),
            empty="No linked decisions found.",
        ),
    )
    body += _section(
        ...
        _table(
            ...
            payload.get("candidate_evaluations", {}).get("items", []),
            empty="No candidate evaluations found.",
        ),
    )
    body += _section(
        "Linked Promotions",
        _table(
            ...
            payload.get("linked_promotions", {}).get("items", []),
            empty="No linked promotions found.",
        ),
    )
...
def _symbol_detail_page(service: DashboardDataService, symbol: str, query: dict[str, list[str]]) -> bytes:
    ...
    payload = service.chart_payload(
        symbol, timeframe=timeframe, lookback=lookback, run_id=run_id, source=source, mode=mode
    )
    trades = service.trades_payload(symbol, run_id=run_id, source=source, mode=mode)
    signals = service.signals_payload(symbol, lookback=lookback, run_id=run_id, source=source, mode=mode)
    body = _metric_cards(
        [
            ("Bars", payload.get("meta", {}).get("bar_count") or 0, f"{lookback} bars requested"),
            ("Signals", payload.get("meta", {}).get("signal_count") or 0, "Selected source"),
            ("Trades", payload.get("meta", {}).get("trade_count") or 0, "Associated ledger"),
            ("Position Qty", payload.get("position", {}).get("qty") or 0, "Latest position snapshot"),
        ]
    )
    ...
    body += _section(
        ...
        _info_cards(
            [
                (
                    "Source",
                    payload.get("meta", {}).get("selected_source") or "latest",
                    _freshness_badge(
                        path=payload.get("meta", {}).get("signal_source") or payload.get("meta", {}).get("trade_source")
                    ),
                ),
                (
                    "Run",
                    payload.get("meta", {}).get("selected_run_id") or "latest",
                    _escape(payload.get("meta", {}).get("selected_mode") or "default mode"),
                ),
                (
                    "Trade Source",
                    payload.get("meta", {}).get("trade_source_mode") or "n/a",
                    _escape(payload.get("meta", {}).get("trade_source")),
                ),
            ]
        ),
    )
    body += (
        ...
        + _section(
            ...
            _table(
                ["trade_id", "symbol", "side", "qty", "entry_ts", "exit_ts", "realized_pnl", "status", "strategy_id"],
                trades.get("trades", []),
                empty="No trades found for this symbol.",
            ),
        )
        + "</div>"
    )
    body += _section(
        "Signals",
        _table(
            ["ts", "type", "label", "price", "score"],
            signals.get("signals", []),
            empty="No signals found for this symbol.",
        ),
    )
    body += _section(
        ...
        _table(
            ["source", "run_id"],
            payload.get("meta", {}).get("available_chart_sources", []),
            empty="No alternate chart sources found.",
        ),
    )
    body += _section(
        "Decision Provenance Rows",
        _table(
            [
                "ts",
                "trade_id",
            ...
            empty="No decision provenance found.",
        ),
    )
...
def create_dashboard_app(
    ...
    *,
    ...
    def app(environ, start_response):
        ...
        runs_filters = _page_state(
            "/runs",
            query,
            filter_keys=["status", "run_kind", "run_type", "mode", "strategy", "date_from", "date_to"],
            default_limit=20,
        ).to_service_filters()
        trades_filters = _page_state(
            "/trades",
            query,
            filter_keys=["status", "decision_status", "strategy", "symbol", "run_id", "date_from", "date_to"],
            default_limit=50,
        ).to_service_filters()
        ops_filters = _page_state(
            "/ops", query, filter_keys=["status", "activity_type", "date_from", "date_to"], default_limit=20
        ).to_service_filters()
        strategy_filters = _page_state(
            "/strategies",
            query,
            filter_keys=["status", "strategy", "decision", "date_from", "date_to"],
            default_limit=20,
        ).to_service_filters()
        if path.startswith("/api/chart/"):
        ...
        elif path == "/api/trades-blotter":
            status, headers, body = _json_response(service.trade_blotter_payload(trades_filters))
        elif path == "/api/ops":
            status, headers, body = _json_response(service.ops_payload(ops_filters))
            ...
            status, headers, body = _json_response(
                {
                    ...
                    "recent_trades": discovery.get("recent_trades", []),
            ...
            status, headers, body = _json_response(
                {
                    "generated_at": discovery.get("generated_at"),
                    "recent_symbols": discovery.get("recent_symbols", []),
                    "summary": discovery.get("summary", {}),
                }
            )
        elif path == "/api/runs":
            status, headers, body = _json_response(service.runs_payload(runs_filters))
            ...
            status, headers, body = (
                _not_found()
                if not run_id
                else _json_response(service.latest_run_detail_payload(run_id, run_kind=_query_value(query, "run_kind")))
            )
        elif path == "/api/daily-trading/latest":
            status, headers, body = _json_response(service.latest_daily_trading_payload())
            ...
            status, headers, body = _json_response(service.strategies_payload(strategy_filters))
        elif path == "/api/strategy-quality/latest":
            status, headers, body = _json_response(service.strategy_quality_payload())
            ...
            status, headers, body = _json_response(
                {
                    ...
                    "drawdown_curve": overview.get("drawdown_curve", []),
            ...
            status, headers, body = (
                _not_found() if not trade_id else _json_response(service.trade_detail_payload(trade_id))
            )
        elif path.startswith("/api/strategies/"):
            strategy_id = path.removeprefix("/api/strategies/")
            status, headers, body = (
                _not_found() if not strategy_id else _json_response(service.strategy_detail_payload(strategy_id))
            )
        elif path == "/":
            ...
            status, headers, body = (
                ...
                if not trade_id
                    ...
                    "200 OK",
                    [("Content-Type", "text/html; charset=utf-8")],
                    _trade_detail_page(service, trade_id, query),
                )
            )
            ...
            status, headers, body = (
                "200 OK",
                [("Content-Type", "text/html; charset=utf-8")],
                _strategies_page(service, query),
            )
            ...
            status, headers, body = (
                ...
                if not strategy_id
                    ...
                    "200 OK",
                    [("Content-Type", "text/html; charset=utf-8")],
                    _strategy_detail_page(service, strategy_id),
                )
            )
        elif path == "/portfolio":
            ...
            status, headers, body = "200 OK", [("Content-Type", "text/html; charset=utf-8")], _ops_page(service, query)
        elif path == "/runs":
            status, headers, body = "200 OK", [("Content-Type", "text/html; charset=utf-8")], _runs_page(service, query)
            ...
            status, headers, body = (
                ...
                if not run_id
                    ...
                    "200 OK",
                    [("Content-Type", "text/html; charset=utf-8")],
                    _run_detail_page(service, run_id, query),
                )
            )
        elif path == "/execution":
...
def build_dashboard_static_data(*, artifacts_root: str | Path, output_dir: str | Path) -> dict[str, Path]:
    ...
    payloads = {
        ...
        "trades_blotter.json": service.trade_blotter_payload(),
        "ops.json": service.ops_payload(),
        "runs.json": service.runs_payload(),
        "runs_latest.json": service.latest_run_detail_payload(),
        "daily_trading_latest.json": service.latest_daily_trading_payload(),
        ...
        "strategy_quality_latest.json": service.strategy_quality_payload(),
