...
def _write_markdown_summary(payload: dict[str, Any], health_checks: list[dict[str, Any]]) -> str:
    lines = [
        ...
        f"- Pre-validation target count: `{payload.get('pre_validation_target_symbol_count', 0)}`",
        f"- Post-validation target count: `{payload.get('post_validation_target_symbol_count', 0)}`",
        f"- Usable symbol count: `{payload.get('usable_symbol_count', 0)}`",
        f"- Skipped symbol count: `{payload.get('skipped_symbol_count', 0)}`",
        f"- Target drop stage: `{payload.get('target_drop_stage')}`",
        f"- Zero target reason: `{payload.get('zero_target_reason')}`",
        f"- Target drop reason: `{payload.get('target_drop_reason')}`",
        f"- Generated preset path: `{payload.get('generated_preset_path')}`",
        f"- Signal artifact path: `{payload.get('signal_artifact_path')}`",
...
def write_live_dry_run_artifacts(
    ...
    *,
    ...
    summary_payload = {
        ...
        "pre_validation_target_symbol_count": target_diagnostics.get("pre_validation_target_symbol_count"),
        "post_validation_target_symbol_count": target_diagnostics.get("post_validation_target_symbol_count"),
        ...
        "target_drop_stage": target_diagnostics.get("target_drop_stage"),
        "zero_target_reason": target_diagnostics.get("zero_target_reason"),
        "target_drop_reason": target_diagnostics.get("target_drop_reason"),
        "generated_preset_path": target_diagnostics.get("generated_preset_path"),
        "signal_artifact_path": target_diagnostics.get("signal_artifact_path"),
    ...
    summary_payload = add_standard_summary_fields(
        ...
        key_counts={
            ...
            "pre_validation_target_symbol_count": int(target_diagnostics.get("pre_validation_target_symbol_count", 0) or 0),
            "post_validation_target_symbol_count": int(target_diagnostics.get("post_validation_target_symbol_count", len(result.target_weights)) or 0),
