...
- Candidate artifacts now persist durable entry-time EV fields for all candidate rows, not just executed trades. That includes `predicted_return`, `ev_weighting_score`, EV model source/fallback fields, and explicit reliability status fields so reliability-history joins stop losing rows unnecessarily.
- Reliability warm-up is now explicit and auditable. Use `reliability_bootstrap_min_training_samples`, `reliability_enabled_after_min_history_rows`, `reliability_enabled_after_min_fit_days`, and `reliability_cold_start_behavior` to control when reliability stays in `cold_start`, transitions through `insufficient_history`, and finally becomes `active`.
- Use `scripts/run_reliability_warmup_grid.py` against a completed replay root to evaluate warm-up policies post hoc without rerunning the full trading pipeline. It writes `replay_ev_reliability_warmup_grid_summary.csv` with readiness, active-day quality, and baseline replay economics for each threshold combination.
- On the current `2025-01-03` through `2025-02-14` replay window, the warm-up grid still produced at most one active fit day across the tested threshold surface. That is enough to prove the persistence/readiness path is fixed, but not enough to recommend enabling reliability broadly yet. The next honest step is a longer replay window, not looser safety thresholds.
